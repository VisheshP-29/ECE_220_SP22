...
We started first by testing if a given node is a leaf node (left and right children are null) or internal node (has at least one child). We then checked if a given
subtree rooted at a given node resides in a different subtree rooted at node a. We basically iterated through all children of a to find node b. Then we implmented
the super simple rotate function, where we simply swapped the width and height of the slicing tree. After rotate, we changed the cutline values, swapping V for H, and 
vice versa. Then we swapped two modules between two given leave nodes in the slicing tree. Then we switched the topology of two given subtrees, after checking if 
one subtree belongs to part of the other. We then implemented the postfix_traversal function, where we iterated through the expression array, and if it had a module,
we set that element to the module ptr points to. Otherwise, the element would be set to the cutline value ptr points to. We then implemented get_resource, where we
recursive went through the tree, summing the individual module resources, before returning them. Finally, we completed the init_slicing_tree function, where we checked
first for the base case (if 'counter' n is at the end). If we are, we know to populate the module, and leave the cutline undefined, and also to set the parent to par.
For any internal node, we set the cutline to V, module to NULL, and parent to par again. Then, we create a right child. If we are creating a right child, that means
that it must hold a module value, so we set the cutline to undefined, module gets populated, and the parent of the child node becomes par. Finally, for the left child,
we simply call the function again, but we've incremented our 'counter' n.
...
int is_leaf_node(node_t* ptr) {
	if(ptr -> left == NULL && ptr->right == NULL)						//if the left and right node are null, then it is a leaf node.
		return 1;
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if(ptr->left != NULL || ptr->right != NULL)						//if neither the left nor right node are null, it is an internal
		return 1;
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
	if(a == NULL)											//check the base case, if a is null
		return 0;
	if(a==b)												//test if a is equal to b
		return 1;
	return is_in_subtree(a->right, b) || is_in_subtree(a->left, b);					//iterate through the nodes left and right of a to see if they
													//match b
  // TODO: (remember to modify the return value appropriately)
...
void rotate(node_t* ptr) {
	int temp;											//initalize temp variable
	temp = ptr->module->w;										//set temp var to the width
	ptr->module->w = ptr->module->h;								//set width to height of module
	ptr->module->h = temp;										//set height to temp(width of module)
  // TODO: 
}
...
void recut(node_t* ptr) {
  ...
  if(ptr->cutline == V)											//if the cutline was V, change it to H
	ptr->cutline = H;
  else													//otherwise, the cutline was H, so change it to V
	ptr->cutline = V;
...
void swap_module(node_t* a, node_t* b) {
  ...
  module_t *temp = (module_t*) malloc(sizeof(module_t));							//create temp pointer to hold module at a
  temp = a->module;											//store node leaf in temp
  a->module = b->module;										//set node leaf of b in a
  b->module = temp;											//set node leaf of a in b
  // TODO:
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t *temp = (node_t*) malloc(sizeof(node_t));							//create temp pointer to hold parent
  temp = a->parent;											//store parent of a in temp
  a->parent = b->parent;										//set parent of b to parent of a
  b->parent = temp;											//set parent of temp (a) to parent of b
  
  if(a->parent->left == b)										//check if the left node was b, 
	a->parent->left = a;										//if it was then a will be the left node
  else													//otherwise, set a to be the right node
	a->parent->right = a;

  if(b->parent->left == a)										//check if the left node was a, 
	b->parent->left = b;										//if it was then b will be the left node
  else													//otherwise, set b to be the right node
	b->parent->right = b;
  // TODO:
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  postfix_traversal(ptr->left, nth, expression);							//go through left subtree
  postfix_traversal(ptr->right, nth, expression);							//go through right subtree
  
  if (ptr->module != NULL){							//if the cutline at the array element isn't undefined
	expression[*nth].module = ptr->module;							//set the array element to cutline
  	expression[*nth].cutline = UNDEFINED_CUTLINE;
  }
  else{
	expression[*nth].module = NULL;								//otherwise, set the array element to module
...
{
  // TODO:
  int sum = 0;												//initalize variable to hold the sum
  if(ptr!= NULL){											//check base case
	if(is_leaf_node(ptr)){										//if we reach a leaf node, add the resource to sum
		sum+= ptr->module->resource;
	}
  	sum += get_total_resource(ptr->left);								//traverse through left subtree
	sum += get_total_resource(ptr->right);								//traverse through right subtree
  }
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t* ptr = (node_t *)malloc(sizeof(node_t));							//create pointer, ptr


  if(n == num_modules -1){										//check if we are at the end
  	ptr->module = modules + n;									//populate module with data provided
	ptr->cutline = UNDEFINED_CUTLINE;								//set the cutline to undefined
	ptr->parent = par;										//set parent to par
  	return ptr;
  }
  //internal node
  ptr->cutline = V;											//for an internal node, set cutline to V
  ptr->module = NULL;											//set module to null
  ptr->parent = par;											//set parent to par

  //right child
  ptr->right = (node_t *)malloc(sizeof(node_t));							//create node for right child of current node
  ptr->right->module = modules + n;									//populate module with data provided
  ptr->right->cutline = UNDEFINED_CUTLINE;								//set cutline to undefined
  ptr->right->parent = ptr;										//set parent node to par

  //left child
  ptr->left = init_slicing_tree(ptr,n+1);								//recursive call
  // TODO:	
